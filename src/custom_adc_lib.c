...
/// handy macro to stop the ADC only if it is running (crashes otherwise)
...
void enable_EOC_interrupt() {
	STOP_ADC_IF_RUNNING()
	; // make sure that there is no conversion in the background
...
void enable_EOC_interrupt_timer(uint16_t time_ms) {
	...
	TIM6->PSC = 15999; // timer should tick in ms
	TIM6->ARR = time_ms;
	TIM6->CR2 |= TIM_CR2_MMS_1; // should be '010' for use with ADC

	NVIC_EnableIRQ(ADC1_COMP_IRQn);
	ADC1->IER |= ADC_IER_EOCIE;
	ADC1->CFGR1 |= ADC_CFGR1_EXTEN_0; // rising edge trigger, default
...
void adc_init() {
	// probably useless, TODO: maybe remove?
//	SYSCFG->CFGR3 |= SYSCFG_CFGR3_ENBUF_SENSOR_ADC
//			| SYSCFG_CFGR3_ENBUF_VREFINT_ADC | SYSCFG_CFGR3_EN_VREFINT;
